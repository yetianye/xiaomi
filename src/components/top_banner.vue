<template>
	<div class='container banner'>
		<div class="banner_menu">
			<ul>
				<li class='menu_item' v-for='item in menu' @mouseenter='evtenter(item.type)' @mouseleave='evtleave'><a href="#">{{item.content}}</a></li>
			</ul>
		</div>
		<div class='menu_detail' v-if='isMenu==true' @mouseenter='evtenter()' @mouseleave='evtleave'>
			<ul v-for='item in handleMemu'>
				<li v-for='i in item'>
					<a href="#">
						<img :src="i.imgUrl" alt="">
						<span>{{i.name}}</span>
					</a>
					<a v-if='i.buyStatus==true' href="#">选购</a>
				</li>
			</ul>
		</div>	
			
		<banner-slide :banners='bannerImg'></banner-slide>
	</div>
</template>
<script>
	import bannerSlide from './top_banner_slide.vue'
	...
	bannerImg:[{sourceUrl: '//item.mi.com/buyphone/mi5', imgUrl: 'http://i3.mifile.cn/a4/bc62a28f-de64-4eee-853b-36772a97f67e'},
		...
		computed:{
			handleMemu(){
				...
				for(var i=0;i<this.currentMenu.length;i++){
					...
					result[index].push(this.currentMenu[i]);
				}
				return result;
			},
			...mapState({
